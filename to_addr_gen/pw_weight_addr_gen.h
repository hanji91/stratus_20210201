...
SC_MODULE(pw_weight_addr_gen)
{
  ...
  */
  ...
  void _ox_ctrl()
  {
    ...
    ox_clear = (ox_enable.read()) && (ox_cnt.read() == ox_cnt_max2.read() - systolic_depth.read()); // Modified on 2021/01/25 by hanji(Bug fixing)
  }
  void _ox_counter()
  {
    if (!rstn.read()) // Added on 2020/12/11 by hanji
  ...
  void _oy_ctrl()
  {
    ...
    oy_clear = (oy_enable.read()) && (oy_cnt.read() == oy_cnt_max2.read() - 1); // Modified on 2021/01/25 by hanji(Bug fixing)
  }
  void _oy_counter()
  {
    if (!rstn.read()) // Added on 2020/12/11 by hanji
