...
#ifndef _ORG_BN_ADDR_GEN_COSIM_INCLUDED_
#define _ORG_BN_ADDR_GEN_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct org_bn_addr_gen_cosim : public ncsc_foreign_module
#else
SC_MODULE(org_bn_addr_gen_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module org_bn_addr_gen is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of org_bn_addr_gen in SystemC\n", 
					scName );
				return;
			} else {
				s << "org_bn_addr_gen0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module org_bn_addr_gen are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "org_bn_addr_gen", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'org_bn_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "org_bn_addr_gen", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'org_bn_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( data_en_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "org_bn_addr_gen", "data_en", scName ) ) {
			esc_report_error( esc_error, "The port 'data_en' on module 'org_bn_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module org_bn_addr_gen must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "org_bn_addr_gen_nc_cosim"; } 
    org_bn_addr_gen_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	org_bn_addr_gen_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" org_bn_addr_gen") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int org_bn_addr_gen_cosim::numLinked = 0;
org_bn_addr_gen_cosim::StringPair org_bn_addr_gen_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
