...
#ifndef _ST_FEATURE_ADDR_GEN_COSIM_INCLUDED_
#define _ST_FEATURE_ADDR_GEN_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct st_feature_addr_gen_cosim : public ncsc_foreign_module
#else
SC_MODULE(st_feature_addr_gen_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module st_feature_addr_gen is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of st_feature_addr_gen in SystemC\n", 
					scName );
				return;
			} else {
				s << "st_feature_addr_gen0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module st_feature_addr_gen are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		esc_link_signals( &dilation_x, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &dilation_y, module_path, sim_domain, true, inputDelay );
		...
		esc_link_signals( &st_feature_addr, module_path, sim_domain, true );
		esc_link_signals( &st_feature_addr_valid, module_path, sim_domain, true );
		esc_link_signals( &st_feature_data_valid, module_path, sim_domain, true );
		esc_link_signals( &cache_en, module_path, sim_domain, true );
		esc_link_signals( &st_feature_data_sel, module_path, sim_domain, true );
		esc_link_signals( &st_addr_if_start, module_path, sim_domain, true );
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "st_feature_addr_gen", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'st_feature_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "st_feature_addr_gen", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'st_feature_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( stop_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "st_feature_addr_gen", "stop", scName ) ) {
			esc_report_error( esc_error, "The port 'stop' on module 'st_feature_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module st_feature_addr_gen must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "st_feature_addr_gen_nc_cosim"; } 
    st_feature_addr_gen_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	st_feature_addr_gen_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" st_feature_addr_gen") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int st_feature_addr_gen_cosim::numLinked = 0;
st_feature_addr_gen_cosim::StringPair st_feature_addr_gen_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
