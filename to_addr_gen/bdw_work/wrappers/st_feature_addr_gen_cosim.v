//
// Verilog co-simulation wrapper module for the st_feature_addr_gen module.
...
// by esc_link_signals calls in the proxy module found in the st_feature_addr_gen_cosim.h 
// file.

`timescale 1 ps / 1 ps

module st_feature_addr_gen_cosim;
	...
	wire[31:0] st_feature_addr;
	wire st_feature_addr_valid;
	wire st_feature_data_valid;
	wire cache_en;
	wire[15:0] st_feature_data_sel;
	wire st_addr_if_start;


	`include "hub.v"

	st_feature_addr_gen st_feature_addr_gen0( .clk(clk),
	 .rstn(rstn),
	 .stop(stop),
	 .start(start),
	...
	always @( st_feature_addr_gen0.clk ) begin
	...
	// Make sure that we process delayed assignments after the clock edge.
	always @( st_feature_addr_gen0.rstn ) begin
	...
	// Make sure that we process delayed assignments after the clock edge.
	always @( st_feature_addr_gen0.stop ) begin
