...
#include	"org_bn_addr_gen_rtl.cpp"
...
#if defined(BDW_RTL) || defined(COWARE_RTL_org_bn_addr_gen)
...
#undef org_bn_addr_gen
#define org_bn_addr_gen org_bn_addr_gen_rtl
#	include	"org_bn_addr_gen_rtl.cpp"
#undef org_bn_addr_gen
...
#else
#include	"org_bn_addr_gen_cosim.h"
#endif
...
#include	"org_bn_addr_gen.cpp"


#define		org_bn_addr_gen_INTERNAL


#include	"org_bn_addr_gen_wrap.h"

// this global pointer indicates whether or not the Verilator cycsim wrapper is present
org_bn_addr_gen_cycsim* (*org_bn_addr_gen_cycsim_factory_p)() = 0;
...
const char * org_bn_addr_gen_wrapper::simConfigName()
{
...
enum org_bn_addr_gen_wrapper::Representation org_bn_addr_gen_wrapper::lookupRepresentation( const char* instName )
{
	...
	int iresult = 0;
	err = qbhGetRepresentation(hProj, "org_bn_addr_gen", simConfig, instName, &iresult );
		...
		if (instName && *instName) {
			...
			esc_report_error( esc_warning, "Instance names were specified for module '%s' in simConfig '%s', but they did not "
			                    "match the actal instance names in the RTL.  Try '%s' or '%s'. Simulating '%s' as BEH.\n",
								 "org_bn_addr_gen", simConfig, instName, leaf,  "org_bn_addr_gen" );
		}
		return BDWRep_Behavioral;
	}
...
{
    enum org_bn_addr_gen_wrapper::Representation rep =
		org_bn_addr_gen_wrapper::lookupRepresentation( name() );

	esc_log_wrapper_inst( "org_bn_addr_gen" );
	esc_log_representation( "org_bn_addr_gen", name(), rep );
	...
	{
		case BDWRep_Gates:
		    org_bn_addr_gen_cosim0 = new org_bn_addr_gen_cosim( "org_bn_addr_gen" );

		    org_bn_addr_gen_cosim0->clk(clk);
		    org_bn_addr_gen_cosim0->rstn(rstn);
		    org_bn_addr_gen_cosim0->out_feature_width(out_feature_width);
		    org_bn_addr_gen_cosim0->out_feature_height(out_feature_height);
		    org_bn_addr_gen_cosim0->out_feature_channel(out_feature_channel);
		    org_bn_addr_gen_cosim0->bn_read_base_addr(bn_read_base_addr);
		    org_bn_addr_gen_cosim0->bn_en(bn_en);
		    org_bn_addr_gen_cosim0->start_rising(start_rising);
		    org_bn_addr_gen_cosim0->data_en(data_en);
		    org_bn_addr_gen_cosim0->bn_addr(bn_addr);
		    org_bn_addr_gen_cosim0->bn_addr_valid(bn_addr_valid);
		...
		case BDWRep_RTL_HDL:
			...
			break;
        case BDWRep_CYC_HDL:
            if ( org_bn_addr_gen_cycsim_factory_p )
                org_bn_addr_gen_cycsim0 = org_bn_addr_gen_cycsim_factory_p();
            else
            {
                esc_report_error( esc_fatal, "verilator_produced files not available for module org_bn_addr_gen");
                return;
            }

            esc_report_error( esc_note, " using verilator model for module \"org_bn_addr_gen\"");

		    org_bn_addr_gen_cycsim0->clk(clk);
		    org_bn_addr_gen_cycsim0->rstn(rstn);
		    org_bn_addr_gen_cycsim0->out_feature_width(out_feature_width);
		    org_bn_addr_gen_cycsim0->out_feature_height(out_feature_height);
		    org_bn_addr_gen_cycsim0->out_feature_channel(out_feature_channel);
		    org_bn_addr_gen_cycsim0->bn_read_base_addr(bn_read_base_addr);
		    org_bn_addr_gen_cycsim0->bn_en(bn_en);
		    org_bn_addr_gen_cycsim0->start_rising(start_rising);
		    org_bn_addr_gen_cycsim0->data_en(data_en);
		    org_bn_addr_gen_cycsim0->bn_addr(bn_addr);
		    org_bn_addr_gen_cycsim0->bn_addr_valid(bn_addr_valid);

			break;
#if defined(BDW_RTL) || defined(COWARE_RTL_org_bn_addr_gen)
		case BDWRep_RTL_C:
		    org_bn_addr_gen_rtl0 = new org_bn_addr_gen_rtl( "org_bn_addr_gen" );

		    org_bn_addr_gen_rtl0->clk(clk);
		    org_bn_addr_gen_rtl0->rstn(rstn);
		    org_bn_addr_gen_rtl0->out_feature_width(out_feature_width);
		    org_bn_addr_gen_rtl0->out_feature_height(out_feature_height);
		    org_bn_addr_gen_rtl0->out_feature_channel(out_feature_channel);
		    org_bn_addr_gen_rtl0->bn_read_base_addr(bn_read_base_addr);
		    org_bn_addr_gen_rtl0->bn_en(bn_en);
		    org_bn_addr_gen_rtl0->start_rising(start_rising);
		    org_bn_addr_gen_rtl0->data_en(data_en);
		    org_bn_addr_gen_rtl0->bn_addr(bn_addr);
		    org_bn_addr_gen_rtl0->bn_addr_valid(bn_addr_valid);


			{
			const char *simConfig = org_bn_addr_gen_wrapper::simConfigName();
...
void org_bn_addr_gen_wrapper::start_of_simulation()
{

#include <org_bn_addr_gen_trace.h>

    esc_multiple_writers_warning();
}

void org_bn_addr_gen_wrapper::CloseTrace()
{
	if ( esc_trace_is_enabled( esc_trace_vcd ) ) {
	esc_close_vcd_trace();
	}
...
void org_bn_addr_gen_wrapper::end_of_simulation()
{
	CloseTrace();
}
...
void org_bn_addr_gen_wrapper::DeleteInstances()
{
    ...
    if (org_bn_addr_gen_cosim0)
    {
        delete org_bn_addr_gen_cosim0;
        org_bn_addr_gen_cosim0 = 0;
    }
    if (org_bn_addr_gen_cycsim0)
    {
        delete org_bn_addr_gen_cycsim0;
        org_bn_addr_gen_cycsim0 = 0;
    }
#if defined(BDW_RTL) || defined(COWARE_RTL_org_bn_addr_gen)
    if (org_bn_addr_gen_rtl0)
    {
        delete org_bn_addr_gen_rtl0;
        org_bn_addr_gen_rtl0 = 0;
    }
#endif
}
...
const char * org_bn_addr_gen_wrapper_r::simConfigName()
{
	static char defineBuf[MAX_SIMCONFIG_LENGTH];
	const char *simConfig = NULL;
	if ( qbhGetDefine( "BDW_SIM_CONFIG", defineBuf, MAX_SIMCONFIG_LENGTH ) == qbhOK )
...
enum org_bn_addr_gen_wrapper_r::Representation org_bn_addr_gen_wrapper_r::lookupRepresentation( const char* instName )
{
	const char *simConfig = simConfigName();
...
void org_bn_addr_gen_wrapper_r::InitInstances()
{
    enum org_bn_addr_gen_wrapper_r::Representation rep =
		org_bn_addr_gen_wrapper_r::lookupRepresentation( name() );
...
void org_bn_addr_gen_wrapper_r::InitThreads()
{
	if ( isBEH() ) 
	{
...
void org_bn_addr_gen_wrapper_r::start_of_simulation()
{
...
void org_bn_addr_gen_wrapper_r::CloseTrace()
{
	if ( esc_trace_is_enabled( esc_trace_vcd ) ) {
	esc_close_vcd_trace();
	}
...
void org_bn_addr_gen_wrapper_r::end_of_simulation()
{
	CloseTrace();
}
...
void org_bn_addr_gen_wrapper_r::DeleteInstances()
{
    if (org_bn_addr_gen0)
    {
        delete org_bn_addr_gen0;
...
class org_bn_addr_gen_wrapper_fsdb_opener {
public:
	org_bn_addr_gen_wrapper_fsdb_opener() {
		esc_set_open_fsdb_trace( esc_open_fsdb_trace );
		esc_set_open_fsdb_scv_trace( esc_open_fsdb_scv_trace );
	}
};

static org_bn_addr_gen_wrapper_fsdb_opener
  org_bn_addr_gen_wrapper_fsdb_opener_inst;
...
void org_bn_addr_gen_wrapper::InitInstances( sc_core::sc_module_name  )
{
	org_bn_addr_gen0 = new org_bn_addr_gen( "org_bn_addr_gen" );
