//
// Verilog co-simulation wrapper module for the pw_feature_addr_gen module.
...
// by esc_link_signals calls in the proxy module found in the pw_feature_addr_gen_cosim.h 
// file.

`timescale 1 ps / 1 ps

module pw_feature_addr_gen_cosim;
	...
	reg[15:0] filter_number;
	reg[31:0] read_feature_base_addr;
	wire[31:0] pw_feature_addr;
	wire pw_feature_addr_valid;
	wire pw_feature_data_valid;
	wire pw_cache_en;
	wire[15:0] pw_feature_data_sel;
	wire pw_addr_if_start;
	wire[7:0] systolic_sel_12d;


	`include "hub.v"

	pw_feature_addr_gen pw_feature_addr_gen0( .clk(clk),
	 .rstn(rstn),
	 .stop(stop),
	 .start(start),
	...
	always @( pw_feature_addr_gen0.clk ) begin
	...
	// Make sure that we process delayed assignments after the clock edge.
	always @( pw_feature_addr_gen0.rstn ) begin
	...
	// Make sure that we process delayed assignments after the clock edge.
	always @( pw_feature_addr_gen0.stop ) begin
