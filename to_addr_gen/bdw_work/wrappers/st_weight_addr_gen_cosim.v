//
// Verilog co-simulation wrapper module for the st_weight_addr_gen module.
...
// by esc_link_signals calls in the proxy module found in the st_weight_addr_gen_cosim.h 
// file.

`timescale 1 ps / 1 ps

module st_weight_addr_gen_cosim;
	...
	wire[31:0] st_weight_addr;
	wire st_weight_addr_valid;
	wire st_weight_data_valid;
	wire cache_en;


	`include "hub.v"

	st_weight_addr_gen st_weight_addr_gen0( .clk(clk),
	 .rstn(rstn),
	 .stop(stop),
	 .start(start),
	...
	always @( st_weight_addr_gen0.clk ) begin
	...
	// Make sure that we process delayed assignments after the clock edge.
	always @( st_weight_addr_gen0.rstn ) begin
	...
	// Make sure that we process delayed assignments after the clock edge.
	always @( st_weight_addr_gen0.stop ) begin
