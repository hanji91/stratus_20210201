...
#include "verilated.h"
#include "Vst_feature_addr_gen_rtl.h"


class Vst_feature_addr_gen_rtl;

class st_feature_addr_gen_cycsimV : public st_feature_addr_gen_cycsim
{
    ...
    sc_signal< uint32_t > dilation_x_I;
    sc_signal< uint32_t > dilation_y_I;
    ...
    sc_signal< uint32_t > st_feature_addr_O;
    sc_signal< uint32_t > st_feature_data_sel_O;
    ...
    inline void thread_dilation_x_I() {
        dilation_x_I = (sc_uint<8>)dilation_x.read();
    };
    inline void thread_dilation_y_I() {
        dilation_y_I = (sc_uint<8>)dilation_y.read();
    };
    ...
    inline void thread_st_feature_addr_O() {
        st_feature_addr = st_feature_addr_O.read();
    };
    inline void thread_st_feature_data_sel_O() {
        st_feature_data_sel = st_feature_data_sel_O.read();
    };


    SC_HAS_PROCESS(st_feature_addr_gen_cycsimV);

public:

    st_feature_addr_gen_cycsimV( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" st_feature_addr_gen") ) )
        : st_feature_addr_gen_cycsim(in_name)
    {
        // instantiate the verilated module
        st_feature_addr_gen_cyc0 = new Vst_feature_addr_gen_rtl( "Vst_feature_addr_gen" );
        // generate port connections
        st_feature_addr_gen_cyc0->clk(clk);
        st_feature_addr_gen_cyc0->rstn(rstn);
        st_feature_addr_gen_cyc0->stop(stop);
        st_feature_addr_gen_cyc0->start(start_I);
        st_feature_addr_gen_cyc0->start_rising(start_rising_I);
        st_feature_addr_gen_cyc0->feature_width(feature_width_I);
        st_feature_addr_gen_cyc0->feature_height(feature_height_I);
        st_feature_addr_gen_cyc0->feature_channel(feature_channel_I);
        st_feature_addr_gen_cyc0->out_feature_width(out_feature_width_I);
        st_feature_addr_gen_cyc0->out_feature_height(out_feature_height_I);
        st_feature_addr_gen_cyc0->out_feature_channel(out_feature_channel_I);
        st_feature_addr_gen_cyc0->dilation_x(dilation_x_I);
        st_feature_addr_gen_cyc0->dilation_y(dilation_y_I);
        st_feature_addr_gen_cyc0->filter_width(filter_width_I);
        st_feature_addr_gen_cyc0->filter_height(filter_height_I);
        st_feature_addr_gen_cyc0->filter_channel(filter_channel_I);
        st_feature_addr_gen_cyc0->stride_x(stride_x_I);
        st_feature_addr_gen_cyc0->stride_y(stride_y_I);
        st_feature_addr_gen_cyc0->pad_top_size(pad_top_size_I);
        st_feature_addr_gen_cyc0->pad_bottom_size(pad_bottom_size_I);
        st_feature_addr_gen_cyc0->pad_left_size(pad_left_size_I);
        st_feature_addr_gen_cyc0->pad_right_size(pad_right_size_I);
        st_feature_addr_gen_cyc0->read_feature_base_addr(read_feature_base_addr_I);
        st_feature_addr_gen_cyc0->st_feature_addr(st_feature_addr_O);
        st_feature_addr_gen_cyc0->st_feature_addr_valid(st_feature_addr_valid);
        st_feature_addr_gen_cyc0->st_feature_data_valid(st_feature_data_valid);
        st_feature_addr_gen_cyc0->cache_en(cache_en);
        st_feature_addr_gen_cyc0->st_feature_data_sel(st_feature_data_sel_O);
        st_feature_addr_gen_cyc0->st_addr_if_start(st_addr_if_start);
        ...
        SC_METHOD( thread_dilation_x_I);
            sensitive << dilation_x;
        SC_METHOD( thread_dilation_y_I);
            sensitive << dilation_y;
        ...
        SC_METHOD( thread_st_feature_addr_O);
            sensitive << st_feature_addr_O;
        SC_METHOD( thread_st_feature_data_sel_O);
            sensitive << st_feature_data_sel_O;


        TraceFileType* tfp = esc_get_ca_trace_file();
        if ( tfp )
        {
            st_feature_addr_gen_cyc0->trace(tfp,99);
        }
    };

    ~st_feature_addr_gen_cycsimV()
    {
        delete st_feature_addr_gen_cyc0;
    }
...
protected:
    Vst_feature_addr_gen_rtl* st_feature_addr_gen_cyc0;
};

st_feature_addr_gen_cycsim* st_feature_addr_gen_cycsim_factory()
{
    return new st_feature_addr_gen_cycsimV("st_feature_addr_gen");
}

class st_feature_addr_gen_linkup
{
public:
    st_feature_addr_gen_linkup() {
        extern st_feature_addr_gen_cycsim* (*st_feature_addr_gen_cycsim_factory_p)();
        st_feature_addr_gen_cycsim_factory_p = &st_feature_addr_gen_cycsim_factory;
    }
};

// Create the one instance of the factory for this verilated model.
st_feature_addr_gen_linkup st_feature_addr_gen_cycsimV_link;
