...
#ifndef _PW_FEATURE_ADDR_GEN_COSIM_INCLUDED_
#define _PW_FEATURE_ADDR_GEN_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct pw_feature_addr_gen_cosim : public ncsc_foreign_module
#else
SC_MODULE(pw_feature_addr_gen_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module pw_feature_addr_gen is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of pw_feature_addr_gen in SystemC\n", 
					scName );
				return;
			} else {
				s << "pw_feature_addr_gen0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module pw_feature_addr_gen are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		esc_link_signals( &filter_number, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &read_feature_base_addr, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &pw_feature_addr, module_path, sim_domain, true );
		esc_link_signals( &pw_feature_addr_valid, module_path, sim_domain, true );
		esc_link_signals( &pw_feature_data_valid, module_path, sim_domain, true );
		esc_link_signals( &pw_cache_en, module_path, sim_domain, true );
		esc_link_signals( &pw_feature_data_sel, module_path, sim_domain, true );
		esc_link_signals( &pw_addr_if_start, module_path, sim_domain, true );
		esc_link_signals( &systolic_sel_12d, module_path, sim_domain, true );
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "pw_feature_addr_gen", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'pw_feature_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "pw_feature_addr_gen", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'pw_feature_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( stop_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "pw_feature_addr_gen", "stop", scName ) ) {
			esc_report_error( esc_error, "The port 'stop' on module 'pw_feature_addr_gen'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module pw_feature_addr_gen must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "pw_feature_addr_gen_nc_cosim"; } 
    pw_feature_addr_gen_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	pw_feature_addr_gen_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" pw_feature_addr_gen") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int pw_feature_addr_gen_cosim::numLinked = 0;
pw_feature_addr_gen_cosim::StringPair pw_feature_addr_gen_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
