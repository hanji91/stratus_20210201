...
#if defined(BDW_RTL) || defined(COWARE_RTL_fp_div)
...
const char * fp_div_wrapper::simConfigName()
{
...
enum fp_div_wrapper::Representation fp_div_wrapper::lookupRepresentation( const char* instName )
{
	...
	int iresult = 0;
	err = qbhGetRepresentation(hProj, "fp_div", simConfig, instName, &iresult );
		...
		if (instName && *instName) {
			...
			esc_report_error( esc_warning, "Instance names were specified for module '%s' in simConfig '%s', but they did not "
			                    "match the actal instance names in the RTL.  Try '%s' or '%s'. Simulating '%s' as BEH.\n",
								 "fp_div", simConfig, instName, leaf,  "fp_div" );
		}
		return BDWRep_Behavioral;
	}
...
{
    enum fp_div_wrapper::Representation rep =
		fp_div_wrapper::lookupRepresentation( name() );

	esc_log_wrapper_inst( "fp_div" );
	esc_log_representation( "fp_div", name(), rep );
	...
	{
		case BDWRep_Gates:
		    fp_div_cosim0 = new fp_div_cosim( "fp_div" );

		    fp_div_cosim0->clk(clk);
		    fp_div_cosim0->rstn(rstn);
		    fp_div_cosim0->src_valid(src_valid);
		    fp_div_cosim0->a_man(a_man);
		    fp_div_cosim0->a_exp(a_exp);
		    fp_div_cosim0->a_sign(a_sign);
		    fp_div_cosim0->b_man(b_man);
		    fp_div_cosim0->b_exp(b_exp);
		    fp_div_cosim0->b_sign(b_sign);
		    fp_div_cosim0->r_man(r_man);
		    fp_div_cosim0->r_exp(r_exp);
		    fp_div_cosim0->r_sign(r_sign);
		    fp_div_cosim0->dst_valid(dst_valid);
		...
		case BDWRep_RTL_HDL:
			...
			break;
        case BDWRep_CYC_HDL:
            if ( fp_div_cycsim_factory_p )
                fp_div_cycsim0 = fp_div_cycsim_factory_p();
            else
            {
                esc_report_error( esc_fatal, "verilator_produced files not available for module fp_div");
                return;
            }

            esc_report_error( esc_note, " using verilator model for module \"fp_div\"");

		    fp_div_cycsim0->clk(clk);
		    fp_div_cycsim0->rstn(rstn);
		    fp_div_cycsim0->src_valid(src_valid);
		    fp_div_cycsim0->a_man(a_man);
		    fp_div_cycsim0->a_exp(a_exp);
		    fp_div_cycsim0->a_sign(a_sign);
		    fp_div_cycsim0->b_man(b_man);
		    fp_div_cycsim0->b_exp(b_exp);
		    fp_div_cycsim0->b_sign(b_sign);
		    fp_div_cycsim0->r_man(r_man);
		    fp_div_cycsim0->r_exp(r_exp);
		    fp_div_cycsim0->r_sign(r_sign);
		    fp_div_cycsim0->dst_valid(dst_valid);

			break;
#if defined(BDW_RTL) || defined(COWARE_RTL_fp_div)
		case BDWRep_RTL_C:
		    fp_div_rtl0 = new fp_div_rtl( "fp_div" );

		    fp_div_rtl0->clk(clk);
		    fp_div_rtl0->rstn(rstn);
		    fp_div_rtl0->src_valid(src_valid);
		    fp_div_rtl0->a_man(a_man);
		    fp_div_rtl0->a_exp(a_exp);
		    fp_div_rtl0->a_sign(a_sign);
		    fp_div_rtl0->b_man(b_man);
		    fp_div_rtl0->b_exp(b_exp);
		    fp_div_rtl0->b_sign(b_sign);
		    fp_div_rtl0->r_man(r_man);
		    fp_div_rtl0->r_exp(r_exp);
		    fp_div_rtl0->r_sign(r_sign);
		    fp_div_rtl0->dst_valid(dst_valid);


			{
			const char *simConfig = fp_div_wrapper::simConfigName();
...
void fp_div_wrapper::start_of_simulation()
{

#include <fp_div_trace.h>

    esc_multiple_writers_warning();
}

void fp_div_wrapper::CloseTrace()
{
	if ( esc_trace_is_enabled( esc_trace_vcd ) ) {
	esc_close_vcd_trace();
	}
...
void fp_div_wrapper::end_of_simulation()
{
	CloseTrace();
}
...
void fp_div_wrapper::DeleteInstances()
{
    ...
    if (fp_div_cosim0)
    {
        delete fp_div_cosim0;
        fp_div_cosim0 = 0;
    }
    if (fp_div_cycsim0)
    {
        delete fp_div_cycsim0;
        fp_div_cycsim0 = 0;
    }
#if defined(BDW_RTL) || defined(COWARE_RTL_fp_div)
    if (fp_div_rtl0)
...
// structured ports
void fp_div_wrapper_r::thread_a()
{
   cynw_cm_float <(int)8, (int)23, (int)1, (int)5, (int)1 > tmp;
   tmp.man = a_man.read();
   tmp.exp = a_exp.read();
...
void fp_div_wrapper_r::thread_b()
{
   cynw_cm_float <(int)8, (int)23, (int)1, (int)5, (int)1 > tmp;
   tmp.man = b_man.read();
   tmp.exp = b_exp.read();
...
void fp_div_wrapper_r::thread_r_man()
{
   r_man = r.read().man;
}
void fp_div_wrapper_r::thread_r_exp()
{
   r_exp = r.read().exp;
}
void fp_div_wrapper_r::thread_r_sign()
{
   r_sign = r.read().sign;
}


const char * fp_div_wrapper_r::simConfigName()
{
	static char defineBuf[MAX_SIMCONFIG_LENGTH];
	const char *simConfig = NULL;
	if ( qbhGetDefine( "BDW_SIM_CONFIG", defineBuf, MAX_SIMCONFIG_LENGTH ) == qbhOK )
...
enum fp_div_wrapper_r::Representation fp_div_wrapper_r::lookupRepresentation( const char* instName )
{
	const char *simConfig = simConfigName();
...
void fp_div_wrapper_r::InitInstances()
{
    enum fp_div_wrapper_r::Representation rep =
		fp_div_wrapper_r::lookupRepresentation( name() );
...
void fp_div_wrapper_r::InitThreads()
{
	if ( isBEH() ) 
	{
		SC_METHOD(thread_a);
...
void fp_div_wrapper_r::start_of_simulation()
{
...
void fp_div_wrapper_r::CloseTrace()
{
	if ( esc_trace_is_enabled( esc_trace_vcd ) ) {
	esc_close_vcd_trace();
	}
...
void fp_div_wrapper_r::end_of_simulation()
{
	CloseTrace();
}
...
void fp_div_wrapper_r::DeleteInstances()
{
    if (fp_div0)
    {
        delete fp_div0;
...
class fp_div_wrapper_fsdb_opener {
public:
	fp_div_wrapper_fsdb_opener() {
		esc_set_open_fsdb_trace( esc_open_fsdb_trace );
		esc_set_open_fsdb_scv_trace( esc_open_fsdb_scv_trace );
	}
};

static fp_div_wrapper_fsdb_opener
  fp_div_wrapper_fsdb_opener_inst;
...
void fp_div_wrapper::InitInstances( sc_core::sc_module_name  )
{
	fp_div0 = new fp_div( "fp_div" );
