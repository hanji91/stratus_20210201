...
#ifndef _RELU_OFFSET_FLOAT_COSIM_INCLUDED_
#define _RELU_OFFSET_FLOAT_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct relu_offset_float_cosim : public ncsc_foreign_module
#else
SC_MODULE(relu_offset_float_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module relu_offset_float is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of relu_offset_float in SystemC\n", 
					scName );
				return;
			} else {
				s << "relu_offset_float0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module relu_offset_float are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		esc_link_signals( &offset_man, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &offset_exp, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &offset_sign, module_path, sim_domain, true, inputDelay );
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "relu_offset_float", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'relu_offset_float'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "relu_offset_float", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'relu_offset_float'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( enable_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "relu_offset_float", "enable", scName ) ) {
			esc_report_error( esc_error, "The port 'enable' on module 'relu_offset_float'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( src_valid_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "relu_offset_float", "src_valid", scName ) ) {
			esc_report_error( esc_error, "The port 'src_valid' on module 'relu_offset_float'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module relu_offset_float must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "relu_offset_float_nc_cosim"; } 
    relu_offset_float_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	relu_offset_float_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" relu_offset_float") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int relu_offset_float_cosim::numLinked = 0;
relu_offset_float_cosim::StringPair relu_offset_float_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
