...
#include "verilated.h"
#include "Vfp_add_rtl.h"


class Vfp_add_rtl;

class fp_add_cycsimV : public fp_add_cycsim
{
    ...
    SC_HAS_PROCESS(fp_add_cycsimV);

public:

    fp_add_cycsimV( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" fp_add") ) )
        : fp_add_cycsim(in_name)
    {
        // instantiate the verilated module
        fp_add_cyc0 = new Vfp_add_rtl( "Vfp_add" );
        // generate port connections
        fp_add_cyc0->clk(clk);
        fp_add_cyc0->rstn(rstn);
        fp_add_cyc0->src_valid(src_valid);
        fp_add_cyc0->a_man(a_man_I);
        fp_add_cyc0->a_exp(a_exp_I);
        fp_add_cyc0->a_sign(a_sign_I);
        fp_add_cyc0->b_man(b_man_I);
        fp_add_cyc0->b_exp(b_exp_I);
        fp_add_cyc0->b_sign(b_sign_I);
        fp_add_cyc0->r_man(r_man_O);
        fp_add_cyc0->r_exp(r_exp_O);
        fp_add_cyc0->r_sign(r_sign_O);
        fp_add_cyc0->dst_valid(dst_valid);
...
protected:
    Vfp_add_rtl* fp_add_cyc0;
};

fp_add_cycsim* fp_add_cycsim_factory()
{
    return new fp_add_cycsimV("fp_add");
}
...
public:
    fp_add_linkup() {
        extern fp_add_cycsim* (*fp_add_cycsim_factory_p)();
        fp_add_cycsim_factory_p = &fp_add_cycsim_factory;
    }
};

// Create the one instance of the factory for this verilated model.
fp_add_linkup fp_add_cycsimV_link;
