...
#if defined(BDW_RTL) || defined(COWARE_RTL_fp_cmp)
...
const char * fp_cmp_wrapper::simConfigName()
{
...
enum fp_cmp_wrapper::Representation fp_cmp_wrapper::lookupRepresentation( const char* instName )
{
	...
	int iresult = 0;
	err = qbhGetRepresentation(hProj, "fp_cmp", simConfig, instName, &iresult );
		...
		if (instName && *instName) {
			...
			esc_report_error( esc_warning, "Instance names were specified for module '%s' in simConfig '%s', but they did not "
			                    "match the actal instance names in the RTL.  Try '%s' or '%s'. Simulating '%s' as BEH.\n",
								 "fp_cmp", simConfig, instName, leaf,  "fp_cmp" );
		}
		return BDWRep_Behavioral;
	}
...
{
    enum fp_cmp_wrapper::Representation rep =
		fp_cmp_wrapper::lookupRepresentation( name() );

	esc_log_wrapper_inst( "fp_cmp" );
	esc_log_representation( "fp_cmp", name(), rep );
	...
	{
		case BDWRep_Gates:
		    fp_cmp_cosim0 = new fp_cmp_cosim( "fp_cmp" );

		    fp_cmp_cosim0->clk(clk);
		    fp_cmp_cosim0->rstn(rstn);
		    fp_cmp_cosim0->enable(enable);
		    fp_cmp_cosim0->src_valid(src_valid);
		    fp_cmp_cosim0->a_man(a_man);
		    fp_cmp_cosim0->a_exp(a_exp);
		    fp_cmp_cosim0->a_sign(a_sign);
		    fp_cmp_cosim0->b_man(b_man);
		    fp_cmp_cosim0->b_exp(b_exp);
		    fp_cmp_cosim0->b_sign(b_sign);
		    fp_cmp_cosim0->r_man(r_man);
		    fp_cmp_cosim0->r_exp(r_exp);
		    fp_cmp_cosim0->r_sign(r_sign);
		    fp_cmp_cosim0->dst_valid(dst_valid);
		...
		case BDWRep_RTL_HDL:
			...
			break;
        case BDWRep_CYC_HDL:
            if ( fp_cmp_cycsim_factory_p )
                fp_cmp_cycsim0 = fp_cmp_cycsim_factory_p();
            else
            {
                esc_report_error( esc_fatal, "verilator_produced files not available for module fp_cmp");
                return;
            }

            esc_report_error( esc_note, " using verilator model for module \"fp_cmp\"");

		    fp_cmp_cycsim0->clk(clk);
		    fp_cmp_cycsim0->rstn(rstn);
		    fp_cmp_cycsim0->enable(enable);
		    fp_cmp_cycsim0->src_valid(src_valid);
		    fp_cmp_cycsim0->a_man(a_man);
		    fp_cmp_cycsim0->a_exp(a_exp);
		    fp_cmp_cycsim0->a_sign(a_sign);
		    fp_cmp_cycsim0->b_man(b_man);
		    fp_cmp_cycsim0->b_exp(b_exp);
		    fp_cmp_cycsim0->b_sign(b_sign);
		    fp_cmp_cycsim0->r_man(r_man);
		    fp_cmp_cycsim0->r_exp(r_exp);
		    fp_cmp_cycsim0->r_sign(r_sign);
		    fp_cmp_cycsim0->dst_valid(dst_valid);

			break;
#if defined(BDW_RTL) || defined(COWARE_RTL_fp_cmp)
		case BDWRep_RTL_C:
		    fp_cmp_rtl0 = new fp_cmp_rtl( "fp_cmp" );

		    fp_cmp_rtl0->clk(clk);
		    fp_cmp_rtl0->rstn(rstn);
		    fp_cmp_rtl0->enable(enable);
		    fp_cmp_rtl0->src_valid(src_valid);
		    fp_cmp_rtl0->a_man(a_man);
		    fp_cmp_rtl0->a_exp(a_exp);
		    fp_cmp_rtl0->a_sign(a_sign);
		    fp_cmp_rtl0->b_man(b_man);
		    fp_cmp_rtl0->b_exp(b_exp);
		    fp_cmp_rtl0->b_sign(b_sign);
		    fp_cmp_rtl0->r_man(r_man);
		    fp_cmp_rtl0->r_exp(r_exp);
		    fp_cmp_rtl0->r_sign(r_sign);
		    fp_cmp_rtl0->dst_valid(dst_valid);


			{
			const char *simConfig = fp_cmp_wrapper::simConfigName();
...
void fp_cmp_wrapper::start_of_simulation()
{

#include <fp_cmp_trace.h>

    esc_multiple_writers_warning();
}

void fp_cmp_wrapper::CloseTrace()
{
	if ( esc_trace_is_enabled( esc_trace_vcd ) ) {
	esc_close_vcd_trace();
	}
...
void fp_cmp_wrapper::end_of_simulation()
{
	CloseTrace();
}
...
void fp_cmp_wrapper::DeleteInstances()
{
    ...
    if (fp_cmp_cosim0)
    {
        delete fp_cmp_cosim0;
        fp_cmp_cosim0 = 0;
    }
    if (fp_cmp_cycsim0)
    {
        delete fp_cmp_cycsim0;
        fp_cmp_cycsim0 = 0;
    }
#if defined(BDW_RTL) || defined(COWARE_RTL_fp_cmp)
    if (fp_cmp_rtl0)
...
// structured ports
void fp_cmp_wrapper_r::thread_a()
{
   cynw_cm_float <(int)8, (int)23, (int)1, (int)5, (int)1 > tmp;
   tmp.man = a_man.read();
   tmp.exp = a_exp.read();
...
void fp_cmp_wrapper_r::thread_b()
{
   cynw_cm_float <(int)8, (int)23, (int)1, (int)5, (int)1 > tmp;
   tmp.man = b_man.read();
   tmp.exp = b_exp.read();
...
void fp_cmp_wrapper_r::thread_r_man()
{
   r_man = r.read().man;
}
void fp_cmp_wrapper_r::thread_r_exp()
{
   r_exp = r.read().exp;
}
void fp_cmp_wrapper_r::thread_r_sign()
{
   r_sign = r.read().sign;
}


const char * fp_cmp_wrapper_r::simConfigName()
{
	static char defineBuf[MAX_SIMCONFIG_LENGTH];
	const char *simConfig = NULL;
	if ( qbhGetDefine( "BDW_SIM_CONFIG", defineBuf, MAX_SIMCONFIG_LENGTH ) == qbhOK )
...
enum fp_cmp_wrapper_r::Representation fp_cmp_wrapper_r::lookupRepresentation( const char* instName )
{
	const char *simConfig = simConfigName();
...
void fp_cmp_wrapper_r::InitInstances()
{
    enum fp_cmp_wrapper_r::Representation rep =
		fp_cmp_wrapper_r::lookupRepresentation( name() );
...
void fp_cmp_wrapper_r::InitThreads()
{
	if ( isBEH() ) 
	{
		SC_METHOD(thread_a);
...
void fp_cmp_wrapper_r::start_of_simulation()
{
...
void fp_cmp_wrapper_r::CloseTrace()
{
	if ( esc_trace_is_enabled( esc_trace_vcd ) ) {
	esc_close_vcd_trace();
	}
...
void fp_cmp_wrapper_r::end_of_simulation()
{
	CloseTrace();
}
...
void fp_cmp_wrapper_r::DeleteInstances()
{
    if (fp_cmp0)
    {
        delete fp_cmp0;
...
class fp_cmp_wrapper_fsdb_opener {
public:
	fp_cmp_wrapper_fsdb_opener() {
		esc_set_open_fsdb_trace( esc_open_fsdb_trace );
		esc_set_open_fsdb_scv_trace( esc_open_fsdb_scv_trace );
	}
};

static fp_cmp_wrapper_fsdb_opener
  fp_cmp_wrapper_fsdb_opener_inst;
...
void fp_cmp_wrapper::InitInstances( sc_core::sc_module_name  )
{
	fp_cmp0 = new fp_cmp( "fp_cmp" );
