...
#include "verilated.h"
#include "Vfp_cmp_rtl.h"


class Vfp_cmp_rtl;

class fp_cmp_cycsimV : public fp_cmp_cycsim
{
    ...
    SC_HAS_PROCESS(fp_cmp_cycsimV);

public:

    fp_cmp_cycsimV( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" fp_cmp") ) )
        : fp_cmp_cycsim(in_name)
    {
        // instantiate the verilated module
        fp_cmp_cyc0 = new Vfp_cmp_rtl( "Vfp_cmp" );
        // generate port connections
        fp_cmp_cyc0->clk(clk);
        fp_cmp_cyc0->rstn(rstn);
        fp_cmp_cyc0->enable(enable);
        fp_cmp_cyc0->src_valid(src_valid);
        fp_cmp_cyc0->a_man(a_man_I);
        fp_cmp_cyc0->a_exp(a_exp_I);
        fp_cmp_cyc0->a_sign(a_sign_I);
        fp_cmp_cyc0->b_man(b_man_I);
        fp_cmp_cyc0->b_exp(b_exp_I);
        fp_cmp_cyc0->b_sign(b_sign_I);
        fp_cmp_cyc0->r_man(r_man_O);
        fp_cmp_cyc0->r_exp(r_exp_O);
        fp_cmp_cyc0->r_sign(r_sign_O);
        fp_cmp_cyc0->dst_valid(dst_valid);
...
protected:
    Vfp_cmp_rtl* fp_cmp_cyc0;
};

fp_cmp_cycsim* fp_cmp_cycsim_factory()
{
    return new fp_cmp_cycsimV("fp_cmp");
}
...
public:
    fp_cmp_linkup() {
        extern fp_cmp_cycsim* (*fp_cmp_cycsim_factory_p)();
        fp_cmp_cycsim_factory_p = &fp_cmp_cycsim_factory;
    }
};

// Create the one instance of the factory for this verilated model.
fp_cmp_linkup fp_cmp_cycsimV_link;
