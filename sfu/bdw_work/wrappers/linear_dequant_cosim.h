...
#ifndef _LINEAR_DEQUANT_COSIM_INCLUDED_
#define _LINEAR_DEQUANT_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct linear_dequant_cosim : public ncsc_foreign_module
#else
SC_MODULE(linear_dequant_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module linear_dequant is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of linear_dequant in SystemC\n", 
					scName );
				return;
			} else {
				s << "linear_dequant0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module linear_dequant are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		esc_link_signals( &divisor_man, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &divisor_exp, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &divisor_sign, module_path, sim_domain, true, inputDelay );
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "linear_dequant", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'linear_dequant'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "linear_dequant", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'linear_dequant'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( enable_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "linear_dequant", "enable", scName ) ) {
			esc_report_error( esc_error, "The port 'enable' on module 'linear_dequant'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( src_valid_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "linear_dequant", "src_valid", scName ) ) {
			esc_report_error( esc_error, "The port 'src_valid' on module 'linear_dequant'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module linear_dequant must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "linear_dequant_nc_cosim"; } 
    linear_dequant_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	linear_dequant_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" linear_dequant") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int linear_dequant_cosim::numLinked = 0;
linear_dequant_cosim::StringPair linear_dequant_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
