...
#include "verilated.h"
#include "Vfloat_div_rtl.h"


class Vfloat_div_rtl;

class float_div_cycsimV : public float_div_cycsim
{
    ...
    SC_HAS_PROCESS(float_div_cycsimV);

public:

    float_div_cycsimV( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" float_div") ) )
        : float_div_cycsim(in_name)
    {
        // instantiate the verilated module
        float_div_cyc0 = new Vfloat_div_rtl( "Vfloat_div" );
        // generate port connections
        float_div_cyc0->clk(clk);
        float_div_cyc0->rstn(rstn);
        float_div_cyc0->enable(enable);
        float_div_cyc0->a_man(a_man_I);
        float_div_cyc0->a_exp(a_exp_I);
        float_div_cyc0->a_sign(a_sign_I);
        float_div_cyc0->b_man(b_man_I);
        float_div_cyc0->b_exp(b_exp_I);
        float_div_cyc0->b_sign(b_sign_I);
        float_div_cyc0->r_man(r_man_O);
        float_div_cyc0->r_exp(r_exp_O);
        float_div_cyc0->r_sign(r_sign_O);
        ...
        if ( tfp )
        {
            float_div_cyc0->trace(tfp,99);
        }
    };

    ~float_div_cycsimV()
    {
        delete float_div_cyc0;
    }
...
protected:
    Vfloat_div_rtl* float_div_cyc0;
};

float_div_cycsim* float_div_cycsim_factory()
{
    return new float_div_cycsimV("float_div");
}

class float_div_linkup
{
public:
    float_div_linkup() {
        extern float_div_cycsim* (*float_div_cycsim_factory_p)();
        float_div_cycsim_factory_p = &float_div_cycsim_factory;
    }
};

// Create the one instance of the factory for this verilated model.
float_div_linkup float_div_cycsimV_link;
