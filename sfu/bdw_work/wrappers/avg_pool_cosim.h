...
#ifndef _AVG_POOL_COSIM_INCLUDED_
#define _AVG_POOL_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct avg_pool_cosim : public ncsc_foreign_module
#else
SC_MODULE(avg_pool_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module avg_pool is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of avg_pool in SystemC\n", 
					scName );
				return;
			} else {
				s << "avg_pool0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module avg_pool are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		esc_link_signals( &out_data_8, module_path, sim_domain, true );
		esc_link_signals( &out_data_9, module_path, sim_domain, true );
		esc_link_signals( &out_data_10, module_path, sim_domain, true );
		esc_link_signals( &out_data_11, module_path, sim_domain, true );
		esc_link_signals( &out_data_12, module_path, sim_domain, true );
		esc_link_signals( &out_data_13, module_path, sim_domain, true );
		esc_link_signals( &out_data_14, module_path, sim_domain, true );
		esc_link_signals( &out_data_15, module_path, sim_domain, true );
		esc_link_signals( &out_data_16, module_path, sim_domain, true );
		esc_link_signals( &out_data_17, module_path, sim_domain, true );
		esc_link_signals( &out_data_18, module_path, sim_domain, true );
		esc_link_signals( &out_data_19, module_path, sim_domain, true );
		esc_link_signals( &out_data_20, module_path, sim_domain, true );
		esc_link_signals( &out_data_21, module_path, sim_domain, true );
		esc_link_signals( &out_data_22, module_path, sim_domain, true );
		esc_link_signals( &out_data_23, module_path, sim_domain, true );
		esc_link_signals( &out_data_24, module_path, sim_domain, true );
		esc_link_signals( &out_data_25, module_path, sim_domain, true );
		esc_link_signals( &out_data_26, module_path, sim_domain, true );
		esc_link_signals( &out_data_27, module_path, sim_domain, true );
		esc_link_signals( &out_data_28, module_path, sim_domain, true );
		esc_link_signals( &out_data_29, module_path, sim_domain, true );
		esc_link_signals( &out_data_30, module_path, sim_domain, true );
		esc_link_signals( &out_data_31, module_path, sim_domain, true );
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "avg_pool", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'avg_pool'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "avg_pool", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'avg_pool'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( enable_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "avg_pool", "enable", scName ) ) {
			esc_report_error( esc_error, "The port 'enable' on module 'avg_pool'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( init_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "avg_pool", "init", scName ) ) {
			esc_report_error( esc_error, "The port 'init' on module 'avg_pool'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( in_data_valid_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "avg_pool", "in_data_valid", scName ) ) {
			esc_report_error( esc_error, "The port 'in_data_valid' on module 'avg_pool'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module avg_pool must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "avg_pool_nc_cosim"; } 
    avg_pool_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	avg_pool_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" avg_pool") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int avg_pool_cosim::numLinked = 0;
avg_pool_cosim::StringPair avg_pool_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
