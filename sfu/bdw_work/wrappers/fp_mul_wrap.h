...
#ifndef _FP_MUL_WRAP_INCLUDED_
#define _FP_MUL_WRAP_INCLUDED_
...
public:
	...
	SC_HAS_PROCESS(fp_mul_wrapper);
		...
		: sc_module(name)
		  ...
		  ,fp_mul0(0), fp_mul_cosim0(0), fp_mul_rtl0(0), fp_mul_cycsim0(0)
    ...
    bool isBEH() { return ( fp_mul_wrapper::lookupRepresentation( name() ) == BDWRep_Behavioral ); }
    bool isRTL_C() { return ( fp_mul_wrapper::lookupRepresentation( name() ) == BDWRep_RTL_C ); }
    bool isRTL_V() { return ( fp_mul_wrapper::lookupRepresentation( name() ) == BDWRep_RTL_HDL ); }
    bool isGATES_V() { return ( fp_mul_wrapper::lookupRepresentation( name() ) == BDWRep_Gates ); }
	bool isCosim() { return ( isRTL_V() || isGATES_V() ); }
    bool isCycsim() { return ( fp_mul_wrapper::lookupRepresentation( name() ) == BDWRep_CYC_HDL ); }

	fp_mul* behModule() { return fp_mul0; }
	fp_mul_cosim* cosimModule() { return fp_mul_cosim0; }
	fp_mul_cycsim* cycsimModule() { return fp_mul_cycsim0; }
	fp_mul_rtl* rtlModule() { return fp_mul_rtl0; }

	fp_mul* fp_mul0;
	fp_mul_cosim* fp_mul_cosim0;
	fp_mul_rtl* fp_mul_rtl0;
	fp_mul_cycsim* fp_mul_cycsim0;
};

// Declaration of wrapper with RTL level ports

SC_MODULE(fp_mul_wrapper_r)
{
public:
	...
	SC_HAS_PROCESS(fp_mul_wrapper_r);

	fp_mul_wrapper_r( sc_module_name name = sc_module_name( sc_gen_unique_name("fp_mul")) )
		: sc_module(name)
		  ,clk("clk")
		  	,rstn("rstn")
		  	,src_valid("src_valid")
    ...
    bool isBEH() { return ( fp_mul_wrapper_r::lookupRepresentation( name() ) == BDWRep_Behavioral ); }
    bool isRTL_C() { return ( fp_mul_wrapper_r::lookupRepresentation( name() ) == BDWRep_RTL_C ); }
    bool isRTL_V() { return ( fp_mul_wrapper_r::lookupRepresentation( name() ) == BDWRep_RTL_HDL ); }
    bool isGATES_V() { return ( fp_mul_wrapper_r::lookupRepresentation( name() ) == BDWRep_Gates ); }
	bool isCosim() { return ( isRTL_V() || isGATES_V() ); }
    bool isCycsim() { return ( fp_mul_wrapper_r::lookupRepresentation( name() ) == BDWRep_CYC_HDL ); }
