...
#ifndef _BIAS_ADD_COSIM_INCLUDED_
#define _BIAS_ADD_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct bias_add_cosim : public ncsc_foreign_module
#else
SC_MODULE(bias_add_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module bias_add is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of bias_add in SystemC\n", 
					scName );
				return;
			} else {
				s << "bias_add0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module bias_add are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		esc_link_signals( &bias_data, module_path, sim_domain, true, inputDelay );
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "bias_add", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'bias_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "bias_add", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'bias_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( enable_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "bias_add", "enable", scName ) ) {
			esc_report_error( esc_error, "The port 'enable' on module 'bias_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( src_valid_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "bias_add", "src_valid", scName ) ) {
			esc_report_error( esc_error, "The port 'src_valid' on module 'bias_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module bias_add must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "bias_add_nc_cosim"; } 
    bias_add_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	bias_add_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" bias_add") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int bias_add_cosim::numLinked = 0;
bias_add_cosim::StringPair bias_add_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
