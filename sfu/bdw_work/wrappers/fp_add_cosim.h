...
#ifndef _FP_ADD_COSIM_INCLUDED_
#define _FP_ADD_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct fp_add_cosim : public ncsc_foreign_module
#else
SC_MODULE(fp_add_cosim)
#endif
{
	struct StringPair {
    ...
    void end_of_elaboration()
    {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module fp_add is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of fp_add in SystemC\n", 
					scName );
				return;
			} else {
				s << "fp_add0" << std::ends;
		...
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module fp_add are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		if ( clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "fp_add", "clk", scName ) ) {
			esc_report_error( esc_error, "The port 'clk' on module 'fp_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( rstn_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "fp_add", "rstn", scName ) ) {
			esc_report_error( esc_error, "The port 'rstn' on module 'fp_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( src_valid_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "fp_add", "src_valid", scName ) ) {
			esc_report_error( esc_error, "The port 'src_valid' on module 'fp_add'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module fp_add must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "fp_add_nc_cosim"; } 
    fp_add_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	fp_add_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" fp_add") ) )
		: sc_module(in_name)
#endif
		  ,clk("clk")
		  	,rstn("rstn")
...
int fp_add_cosim::numLinked = 0;
fp_add_cosim::StringPair fp_add_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
